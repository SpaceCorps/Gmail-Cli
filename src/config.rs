//! Non-secret account metadata in a readable YAML file. Secrets and refresh tokens never
//! land here - those go to [`crate::secrets`].
//!
//! The file layout and location match the .NET CLI this replaced, so an existing
//! `config.yaml` and keystore entries keep working.
...
impl Config {
    /// Account lookup is case-insensitive. Matches by account name first, then by email address.
    pub fn find(&self, name_or_email: &str) -> Option<(&String, &AccountConfig)> {
        if let Some(entry) = self.accounts.iter().find(|(k, _)| k.eq_ignore_ascii_case(name_or_email)) {
            return Some(entry);
        }
        self.accounts.iter().find(|(_, v)| v.email.eq_ignore_ascii_case(name_or_email))
    }
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("GMAIL_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("gmail-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("gmail-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: gmail setup", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another gmail process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    #[test]
    fn finds_by_name_and_email() {
        let mut config = Config::default();
        config.accounts.insert(
            "work".into(),
            AccountConfig {
                email: "work@example.com".into(),
                scope_profile: "draft".into(),
                client_ref: "default".into(),
                added_at: "2026-01-01T00:00:00Z".into(),
            },
        );

        assert!(config.find("work").is_some());
        assert!(config.find("WORK").is_some());
        assert!(config.find("work@example.com").is_some());
        assert!(config.find("WORK@EXAMPLE.COM").is_some());
        assert!(config.find("personal").is_none());
    }
}
